...
// A single ordered strip of the most recently watched titles -- movies and
// shows interleaved by when they were last watched, newest on the left, the way
// the major streamers order "Continue Watching". `sortAt` (last-watched epoch
// ms) is set server-side; items arrive already sorted, this just renders them.
export type RecentItem =
  | { kind: "movie"; sortAt: number; movie: Movie; progressSeconds: number; runtimeMinutes: number | null }
  | { kind: "show"; sortAt: number; show: TVShow };
...
const CARD_CLASS =
  "movie-card group relative block w-[160px] sm:w-[180px] md:w-[240px] overflow-hidden rounded bg-netflix-dark touch-manipulation max-md:rounded-2xl max-md:shadow-lg max-md:shadow-black/40 max-md:ring-1 max-md:ring-white/5";

export function RecentlyWatchedRow({ items }: RecentlyWatchedRowProps) {
  if (items.length === 0) {
    return (
    ...
    <ScrollableRow title="Recently Watched">
      {items.map((item) =>
        item.kind === "movie" ? (
          ...
          <ShowCard key={`s-${item.show.id}`} show={item.show} />
        )
      )}
    </ScrollableRow>
  );
...
function ShowCard({ show }: { show: TVShow }) {
  return (
    <div className={CARD_CLASS}>
      <div className="relative aspect-video w-full">
